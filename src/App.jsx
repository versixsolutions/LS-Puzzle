...
export default function App() {
  ...
  const [isShuffling, setIsShuffling] = useState(false) // ← FIX: Estado global
  
  const canvasRef = useRef(null)
  const bgMusicRef = useRef(null)
  const applauseRef = useRef(null)
  const hasInitialized = useRef(false) // ← FIX: Controle de inicialização
              ...
              <div
                ...
                onClick={() => {
                  if (idx === 0 || levelProgress[idx - 1].stars > 0) {
                    playBeep()
                    setCurrentLevel(idx)
                    hasInitialized.current = false // ← FIX: Reset flag
                    setScreen('game')
                  }
                }}
              >
  ...
  const GameScreen = () => {
    ...
    // ← FIX: isShuffling removido (agora é global)

    // ← FIX: useEffect corrigido com hasInitialized
    useEffect(() => {
      if (!hasInitialized.current && !isShuffling) {
        hasInitialized.current = true
        initializePuzzle()
      }
    }, []) // ← Executa UMA VEZ na montagem

    // ← FIX: Reset ao trocar de nível
    useEffect(() => {
    ...
    const initializePuzzle = () => {
      ...
      img.onload = () => {
        ...
        // ← FIX: Adicionar geração de canvas e imagens
        ...
        for (let row = 0; row < rows; row++) {
          for (let col = 0; col < cols; col++) {
            ...
            newPieces.push({
              ...
              currentRow: row,
              currentCol: col,
              image: pieceCanvas.toDataURL(), // ← FIX: Imagem gerada!
              isPlaced: false
            })
          }
        }
